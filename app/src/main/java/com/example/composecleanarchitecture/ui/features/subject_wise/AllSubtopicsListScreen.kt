...
import com.example.composecleanarchitecture.ui.features.subject_wise.components.SubTopicsCardComponents
...
fun AllSubtopicsListScreen(
    ...
    val courseDataItemList: LazyPagingItems<SubTopicsListResponseDataX> =
        loginViewModel.getAllSubTopics(topicsCode = topicsCode, subjectCode = subjectCode)
            ...
            LazyVerticalGrid(
                ...
                items(
                    ...
                    Box(modifier = Modifier.clickable {
                        item?.let {
                            val subtopicCode = it.attributes?.code
                            val queryCode = "$subjectCode$topicsCode$subjectCode"
                            navController.navigate("TopicsWiseQuestionListScreen/$queryCode/$subtopicCode")
                        }

                    }) {
                        item?.let {
                            SubTopicsCardComponents(
                                it
                            )
                        }
                    }
