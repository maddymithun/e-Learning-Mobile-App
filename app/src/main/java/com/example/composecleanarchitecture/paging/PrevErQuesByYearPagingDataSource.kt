...
import com.example.composecleanarchitecture.data.repository.year_wise_question.IYearWiseQuestRepository
...
class PrevErQuesByYearPagingDataSource(
    private val year: Int,
    private var repository: IYearWiseQuestRepository,
    ...
    val finalList: ArrayList<FormatQuestionsAsExpected> = ArrayList()
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, FormatQuestionsAsExpected> {
        return try {
            ...
            if (isApiCall) {
                val outletResponse = repository.getPrevErQuesByYearList(
                    year = year,
                    page = nextPage
                )
                ...
                if (resultList.isNullOrEmpty()) {
                    roomHelper.getRoomDbInstance().questionCategoryDao()
                        .deletePvsErQuestionAsExpectedFormat(queryCode = year.toString())
                    roomHelper.getRoomDbInstance().questionCategoryDao()
                        .insertPvsErQuestionAsExpectedFormat(finalList)
                    preferencesHelper.putString(
                        "$PREVIOUS_ER_QUES_USING_YEAR$year",
                        "1"
                    )
                }
            } else {
                resultList.clear()
                val response =
                    repository.getPrevErQuesByYearListRoom(
                        queryCode = "$year",
                        params.loadSize,
